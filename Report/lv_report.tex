...
\section{Phạm vi nghiên cứu và nhiệm vụ}
	...
	\begin{itemize}
		\item Phạm vi kĩ thuật:
			\begin{itemize}
				\item Tập trung vào việc điều khiển nhiệt độ trong chamber và đo quang học bằng laser diode – photo diode.
...
\section{Linh kiện chính trên hệ thống}
\subsection{CM4 (Computer Module 4)}

CM4 (Compute Module 4) là máy tính nhúng dạng module do Raspberry Pi phát triển. Module tích hợp vi xử lý Broadcom BCM2711 với kiến trúc ARM Cortex-A72 (64-bit), gồm 4 nhân, xung nhịp tối đa 1.5 GHz, đáp ứng tốt các ứng dụng tính toán nhúng hiệu năng cao.

CM4 được trang bị RAM 2 GB và bộ nhớ eMMC 32 GB, cho phép cài đặt hệ điều hành Linux và lưu trữ chương trình, dữ liệu cũng như các file hệ thống. Module hỗ trợ đa dạng các giao thức giao tiếp phổ biến như Ethernet, WiFi, UART, SPI, I2C, giúp dễ dàng kết nối với các ngoại vi và phần cứng mở rộng.

\begin{figure}[H]
	\centering
	\begin{subfigure}[t]{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{lv_cm4_linhkien_top.jpg}
		\caption{Mặt trên}
		\label{fig:cm4-top}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.48\linewidth}
		\centering
		\includegraphics[width=\linewidth]{lv_cm4_linhkien_bottom.jpg}
		\caption{Mặt dưới}
		\label{fig:cm4-bottom}
	\end{subfigure}
	\caption{CM4 (Computer Module 4)}
	\label{fig:cm4}
\end{figure}

Hình \ref{fig:cm4} minh họa cấu trúc mặt trên và mặt dưới của module CM4, thể hiện các linh kiện chính và hệ thống chân kết nối phục vụ tích hợp vào mạch.

\subsection{Laser diode}

Laser diode là một linh kiện bán dẫn đóng vai trò nguồn phát ánh sáng trong các hệ thống quang học. Linh kiện này có khả năng phát ra ánh sáng laser với các đặc tính nổi bật như đơn sắc, độ định hướng cao và mật độ năng lượng lớn. Nguyên lý hoạt động của laser diode dựa trên quá trình tái hợp của các hạt mang điện trong tiếp giáp P–N. Khi được phân cực thuận, các electron và hole được tiêm vào vùng tiếp giáp, kết hợp với nhau và giải phóng năng lượng dưới dạng photon, tạo ra bức xạ ánh sáng.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{lv_laser_operating_principle_2.png}
	\caption{Nguyên lý hoạt động của laser diode}
	\label{fig:lv_laser_operating_principle_2}
\end{figure}

Đặc điểm quan trọng của laser diode là cường độ phát xạ ánh sáng phụ thuộc trực tiếp vào dòng điện cấp, cho phép điều chỉnh và kiểm soát mức công suất phát một cách linh hoạt. Nhờ khả năng phát xạ ổn định, hội tụ tốt và bước sóng xác định, laser diode thường được sử dụng trong các hệ thống đo lường quang học và thí nghiệm sinh hóa yêu cầu độ chính xác cao.

Trong hệ thống này, laser diode được sử dụng làm nguồn sáng chiếu xuyên qua mẫu vật sinh hóa, tạo điều kiện để quan sát và phân tích sự suy giảm hoặc biến đổi của cường độ ánh sáng khi đi qua mẫu.

\subsection{Photo diode}

Photo diode là một linh kiện bán dẫn được thiết kế để chuyển đổi năng lượng ánh sáng thành tín hiệu điện, hoạt động dựa trên hiệu ứng quang điện. Cấu trúc cơ bản của photo diode là một tiếp giáp P–N, trong đó vùng nghèo đóng vai trò quan trọng trong quá trình thu nhận ánh sáng.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{lv_photo_operating_principle.png}
	\caption{Nguyên lý hoạt động của photo diode}
	\label{fig:lv_photo_operating_principle}
\end{figure}

Khi ánh sáng chiếu vào photo diode, các photon có năng lượng đủ lớn sẽ bị hấp thụ trong vật liệu bán dẫn và tạo ra các cặp electron–hole. Nếu quá trình hấp thụ xảy ra trong hoặc gần vùng nghèo của tiếp giáp P–N, điện trường nội tại trong vùng này sẽ nhanh chóng tách các hạt mang điện về hai phía điện cực, hình thành nên dòng quang điện. Dòng quang điện sinh ra có giá trị tỉ lệ với cường độ ánh sáng chiếu vào photo diode.

Nhờ đặc tính đáp ứng nhanh và quan hệ tuyến tính giữa dòng quang điện và cường độ ánh sáng, photo diode được sử dụng rộng rãi trong các hệ thống đo lường quang học. Trong hệ thống thí nghiệm, photo diode đóng vai trò thu nhận cường độ ánh sáng sau khi xuyên qua mẫu vật, cung cấp thông tin đầu vào cho quá trình phân tích và đánh giá kết quả thí nghiệm.

\subsection{TEC (Thermoelectric Cooler)}

TEC (Thermoelectric Cooler), hay còn gọi là sò nóng lạnh, là một linh kiện bán dẫn hoạt động dựa trên hiệu ứng Peltier. Hiệu ứng này xảy ra khi một dòng điện một chiều chạy qua mối nối của hai vật liệu bán dẫn khác loại, làm xuất hiện hiện tượng hấp thụ nhiệt tại một phía và tỏa nhiệt tại phía còn lại. Do đó, khi cấp dòng điện cho TEC, giữa hai mặt của linh kiện sẽ hình thành một sự chênh lệch nhiệt độ, với một mặt trở thành mặt lạnh và mặt kia trở thành mặt nóng.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\linewidth]{lv_tec_linhkien.jpg}
	\caption{TEC(Thermoelectric Cooler)}
	\label{fig:lv_tec_linhkien}
\end{figure}

\begin{figure}[H]
	\centering
	\includegraphics[width=0.6\linewidth]{lv_tec.png}
	\caption{Nguyên lý hoạt động của TEC}
	\label{fig:lv_tec}
\end{figure}

Đặc điểm quan trọng của TEC là khả năng đảo chiều quá trình truyền nhiệt. Khi đảo chiều dòng điện chạy qua, vai trò của hai mặt sẽ hoán đổi, mặt nóng trở thành mặt lạnh và ngược lại. Nhờ đó, TEC có thể được sử dụng linh hoạt cho cả hai chức năng làm mát và gia nhiệt.

Trong hệ thống thí nghiệm, TEC được sử dụng để điều khiển và ổn định nhiệt độ của buồng thí nghiệm theo giá trị đặt trước, nhằm tạo môi trường nhiệt phù hợp cho quá trình nghiên cứu mẫu vật sinh hóa. Để TEC hoạt động hiệu quả và đảm bảo độ bền, mặt nóng của TEC cần được tản nhiệt tốt nhằm hạn chế hiện tượng quá nhiệt và suy giảm hiệu suất.

\subsection{NTC (Negative Temperature Coefficient)}

NTC (Negative Temperature Coefficient) là một loại điện trở nhiệt có đặc tính điện trở giảm khi nhiệt độ tăng. Hiện tượng này xuất phát từ bản chất bán dẫn của vật liệu cấu thành NTC, trong đó khi nhiệt độ tăng, mật độ hạt mang điện tự do tăng lên, làm giảm điện trở của linh kiện.

\begin{figure}[H]
	\centering
	\begin{subfigure}[t]{0.2\linewidth}
		\centering
		\includegraphics[width=\linewidth]{lv_ntc_linhkien.png}
		\caption{}
		\label{fig:ntc_1}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.25\linewidth}
		\centering
		\includegraphics[width=\linewidth]{lv_ntc_linhkien2.png}
		\caption{}
		\label{fig:ntc_2}
	\end{subfigure}
	\caption{NTC(Negative Temperature Coefficient)}
	\label{fig:ntc}
\end{figure}

Mối quan hệ giữa điện trở và nhiệt độ của NTC là phi tuyến, với sự thay đổi điện trở mạnh theo nhiệt độ trong dải làm việc. Do đặc tính phi tuyến này, không thể mô tả chính xác mối quan hệ điện trở–nhiệt độ bằng các biểu thức tuyến tính đơn giản. Thay vào đó, các nhà sản xuất thường cung cấp các mô hình thực nghiệm để xấp xỉ đặc tuyến của NTC.

Một trong những mô hình phổ biến nhất là mô hình hằng số Beta (B), trong đó mối quan hệ giữa điện trở và nhiệt độ của NTC được biểu diễn bởi công thức:
...
Nhờ mô hình này, giá trị nhiệt độ có thể được xác định từ điện trở của NTC với độ chính xác phù hợp cho các ứng dụng đo và giám sát nhiệt độ trong hệ thống thí nghiệm.
...
\begin{itemize}
	...
	MCU - STM32H7 đóng vai trò lõi điều khiển của hệ thống. Vi điều khiển giao tiếp với CM4 (Raspberry Pi Compute Module 4) qua hai đường tín hiệu cách ly là UART và SPI, MCU chịu trách nhiệm nhận lệnh từ CM4 và điều khiển các bo ngoại vi.
...
\begin{itemize}
	...
	\item Khối Transimpedance Amplifier \& Anti-Aliasing Filter
	
	Do dòng tín hiệu sinh ra từ photo diode thường có biên độ rất nhỏ (ở mức µA hoặc nA), khối xử lý tín hiệu này thực hiện chức năng chuyển đổi dòng quang điện thành điện áp thông qua mạch khuếch đại chuyển đổi dòng–áp (Transimpedance Amplifier), đồng thời khuếch đại tín hiệu lên mức phù hợp để đo lường.
	
	Sau khi được khuếch đại, tín hiệu tiếp tục được đưa qua bộ lọc chống lấy mẫu sai (Anti-aliasing Filter) nhằm loại bỏ các thành phần tần số cao vượt quá tần số Nyquist. Nhờ đó, tín hiệu đưa vào ADC được đảm bảo sạch và hạn chế hiện tượng méo phổ trong quá trình lấy mẫu.
...
\textbf{Khối giám sát nhiệt độ}
	...
	\begin{figure}[H]
		...
		\caption{Sơ đồ nguyên lý mạch đo NTC trên EXP}
...
\textbf{Khối điều khiển nhiệt độ}
	...
	\begin{figure}[H]
		...
		\caption{Sơ đồ nguyên lý mạch điều khiển TEC trên EXP}
	...
	\begin{figure}[H]
		...
		\caption{Sơ đồ nguyên lý mạch điều khiển Heater trên EXP}
...
\textbf{Khối bộ nhớ}
	...
	\begin{figure}[H]
		...
		\caption{Sơ đồ nguyên lý FRAM trên EXP}
...
\textbf{Khối nguồn dòng}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\linewidth]{lv_current_src_correl.png}
...
\textbf{Khối Transimpedance Amplifier \& Anti-Aliasing Filter}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=1\linewidth]{lv_transimpedance_antialias_corel.png}
		\caption{Sơ đồ nguyên lý mạch transimpedance amplifier và mạch anti-aliasing filter trên bo Photo}
		\label{fig:sche-transimpedance-antialias-photo}
	\end{figure}
	
	Mạch transimpedance amplifier là mạch khuếch đại và chuyển đổi dòng điện thành điện áp.
	Nguyên lý hoạt động: 
	Khi chưa có ánh sáng, photo diode không sinh dòng quang. Nhờ hồi tiếp âm qua điện trở R1, 
	op-amp giữ điện áp tại chân (-) xấp xỉ bằng điện áp chân (+) là Vref, đồng thời phân cực ngược cho photo diode.
	
	Khi có ánh sáng chiếu vào, photo diode sinh dòng quang Ipd chảy từ cathode sang anode. Do trở kháng đầu vào của op-amp rất lớn, toàn bộ dòng quang buộc phải đi qua điện trở hồi tiếp Rf.
	Khi đó, điện áp ngõ ra của mạch được xác định theo công thức:
	...
	Điện áp ngõ ra của mạch Transimpedance Amplifier đi qua hai mạch lọc thông thấp RC đơn giản rồi đến một mạch khuếch đại đệm sử dụng op-amp U3C. Sau đó tín hiệu đi qua mạch lọc chống chồng chập tần số (Anti-Aliasing Filter) trước khi đi vào khối ADC.
	...
	Mạch Anti-Aliasing Filter là mạch lọc thông thấp tích cực bậc 2 sử dụng RC và opamp, có nhiệm vụ lọc bỏ tín hiệu có tần số cao, tránh gây ra hiện tượng chồng chập tần số khi lấy mẫu. Tần số cắt -3dB bằng chính xác tần số Nyquisst, bằng một nửa tần số lấy mẫu mong muốn. Trên bo Photo, khối ADC sẽ có tần số lấy mẫu $f_{s}=500kHz$ do đó, $f_{Nyquist}=250kHz$.
...
\begin{itemize}
	...
	Task chịu trách nhiệm giám sát và điều khiển nhiệt độ của buồng thí nghiệm bằng thuật toán PID (Proportional–Integral–Derivative). Nhiệt độ đo được từ cảm biến NTC chính (primary) được sử dụng làm giá trị phản hồi (feedback) để so sánh với nhiệt độ đặt (setpoint), từ đó tính toán sai lệch nhiệt độ và sinh tín hiệu điều khiển tương ứng.
	
	Cảm biến NTC phụ (secondary) được sử dụng làm kênh giám sát độc lập nhằm kiểm tra tính hợp lệ của phép đo. Nếu độ chênh lệch nhiệt độ giữa hai cảm biến vượt quá ngưỡng cho phép, hệ thống sẽ phát hiện trạng thái bất thường và đưa ra cảnh báo lỗi.
	
	Thuật toán PID tạo ra tín hiệu điều khiển liên tục, cho phép điều chỉnh mức làm lạnh hoặc gia nhiệt thông qua TEC và Heater nhằm đưa nhiệt độ buồng thí nghiệm tiến dần và duy trì ổn định tại giá trị đặt trước. Nhờ cơ chế điều khiển phản hồi này, hệ thống đạt được độ chính xác và độ ổn định nhiệt cao hơn so với phương pháp điều khiển theo ngưỡng.
...
\subsubsection{Cả hệ thống}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.5\linewidth]{lv_system_SH.JPG}
...
%	\end{figure}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[angle=90, width=0.8\linewidth]{lv_app_run_nhiet_pid_add_2_sensor.png}
	...
	\begin{figure}[H]
		\centering
		\includegraphics[, width=0.8\linewidth]{lv_draw_photo_adc.png}
